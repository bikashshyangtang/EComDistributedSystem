...
# Loose coupling
    services never share a database or internal models
# Data ownership
    each service has its own SQLite database and EF Core DbContext
# Async-first 
    all EF Core queries use async/await patterns
# Observable 
    Swagger UI is enabled on every service for easy testing
